...
// resident store once, assembles the operator cascade from the user's profile
// weights, runs it, and serializes the feed + trace to JSON. The WASM build binds
// `recommend_from_profile`/`to_json` straight through to JS via embind; the native
// build calls the same functions from main.cpp. One orchestration, two front doors.
// -----------------------------------------------------------------------------
...
// built exactly once, lazily, on the first recommend_from_profile().
...
// Build a query vector from per-category weights: the weighted sum of centroids,
...
// The result of one recommend_from_profile() call: the label, the final feed, and
...
// return the feed + trace. The public entry point recommend_from_profile() builds
// the query from the user's profile weights and calls this; factoring the cascade
// out here keeps query construction separate from pipeline assembly.
inline Recommendation run_recommendation(std::vector<float> query,
    ...
    // Final stage(s). With no exploration floor and no seen set to balance, RerankOp
    // ranks straight to the page (RerankOp -> kPageSize). The profile path adds MixOp
    // after a wider diverse rerank pool: "mostly new + a few strong seen" (new_ratio)
    // PLUS a guaranteed exploration floor drawn from outside the dominant category. The
    // mix runs whenever there is an exploration floor to enforce or a seen set to
    // balance — and the profile path always passes explore_floor > 0, so it always
    // mixes (even a first, unclicked feed still gets its exploration slots).
...
// profile — the SOLE recall entry point (v1's fixed personas and item-based recall
// were removed). The frontend collapses its tag weights into per-category weights
// (indexed exactly like CATEGORY_NAMES) and passes them here; the vector-space math
// stays in C++ (make_query). This is the v2 "profile vector = recall query" entry point.
