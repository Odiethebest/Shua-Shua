...
**为何把 trace 集中在基类：** `{name, in, out, latency_us, sample_ids}` 契约对每个阶段以完全相同的
方式产出，不会漂移也不会被遗漏。trace 是产物，因此在一处强制保证。
...
- **RecallOp**（`recall_op.hpp`）——源头阶段。按与 query 的点积相似度给 store 中每个 item 打分并保留
  ...
  选择器（标量或 SIMD）。
- **FeatureOp**（`feature_op.hpp`）——为每个候选附加特征：`category_match`（persona 对该 item 品类的
  亲和度）、`recency`（`age_days` 的指数衰减）、`popularity`（透传）。基数不变。
...
- **RerankOp**（`rerank_op.hpp`）——贪心 MMR（最大边际相关），以品类为冗余度轴：
  `value = λ·score − (1−λ)·redundancy`，使最终页在相关性与品类多样性之间权衡。
...
- `personas()`——可切换的 personas（标签 + 按品类兴趣权重）。
- `shared_data()`——常驻 item store，只构建一次（函数局部静态）并跨请求复用。
- `recommend(personaId)`——构建 query、组装流水线、运行，返回 `{ persona_label, feed, trace }`。
- `to_json(rec)`——对 feed + trace 的手写 JSON 序列化（前端消费的形状）。保持零依赖。

`bindings.cpp` 通过 embind 向 JavaScript 暴露 `recommend`（返回 JSON 字符串）、`personaCount`、
`personaLabel`。绑定中没有任何引擎逻辑。
...
- **保持 trace 契约。** `{name, in_count, out_count, latency_us, sample_ids}` 固定不变；UI 依赖它。
