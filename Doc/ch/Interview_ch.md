# Interview — 把引擎讲出来（学习与面试向 · 中文版）

`Doc/en/Interview.md` 的中文对照版，方便阅读与面试复习。每一节都用大白话讲清楚：**这块做什么、
关键设计决策与为什么（WHY）、取舍、时间/空间复杂度、以及面试官可能追问的术语**。读完这份
应该能把代码"讲出来"。顺序大致是"基础在前"：先讲引擎内核，再讲建立在其上的行为特性。形式化的
数学（LaTeX 推导、数值分析、复杂度）见 [Algorithm](Algorithm_ch.md)。
...
1. [物品存储 —— 结构体数组 Structure-of-Arrays (SoA)](#1-物品存储--结构体数组-structure-of-arrays-soa)
2. [合成数据 —— 类目质心 + 噪声](#2-合成数据--类目质心--噪声)
3. [召回 Recall —— 基于向量相似度的候选生成](#3-召回-recall--基于向量相似度的候选生成)
4. [算子 DAG —— 统一算子、调度器、trace](#4-算子-dag--统一算子调度器trace)
5. [SIMD 召回内核 + naive/SIMD 一致性校验](#5-simd-召回内核--naivesimd-一致性校验)
6. [Item-based recall —— "看更多类似的"](#6-item-based-recall--看更多类似的)
7. [用户画像 & 隐式反馈 (v2 · B1)](#7-用户画像--隐式反馈-v2--b1)
8. [冷启动 & 标签选择器 (v2 · B2)](#8-冷启动--标签选择器-v2--b2)
9. [实时画像 + 隐式反馈累积 (v2 · B3)](#9-实时画像--隐式反馈累积-v2--b3)
10. [兴趣衰减 (v2 · B4)](#10-兴趣衰减-v2--b4)
11. [画像向量 = 召回 query (v2 · B5)](#11-画像向量--召回-query-v2--b5)
12. [刷新 + 新/已看混合 —— 探索/利用 (v2 · B6)](#12-刷新--新已看混合--探索利用-v2--b6)
13. [可观测性：trace 面板 + 为什么延迟显示为 0 (v2 · B7)](#13-可观测性trace-面板--为什么延迟显示为-0-v2--b7)
14. [会话控制 —— "重新开始" & 记住我 (v2 · B8)](#14-会话控制--重新开始--记住我-v2--b8)

## 1. 物品存储 —— 结构体数组 Structure-of-Arrays (SoA)
...
## 4. 算子 DAG —— 统一算子、调度器、trace

### 4.1 万物皆算子（everything is an operator）
...
完全一致（`diff = 0`）**，并报告最大分数差（2.98e-07，来自求和顺序重排，以及 `fmla` 把乘加
融合成单次舍入）与加速比（扫描 3.09×；**端到端只有 1.57×**，因为共享的 top-k 排序没有被向量化）。

数字来自 `bench/RESULTS.md` 里钉住的那次官方运行。同一次运行还量化了下一步优化：
top-k 排序占 `RecallOp` 的 **81.6%**，换成 `nth_element` 能让 recall 再快 **2.69×** ——
比 SIMD 带来的还多。
...
## 13. 可观测性：trace 面板 + 为什么延迟显示为 0 (v2 · B7)
