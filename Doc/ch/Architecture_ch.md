...
| 组件 | 位置 | 职责 |
|---|---|---|
| Pipeline | `src/pipeline.hpp` | 以节点形式持有算子，按顺序执行，将每一级输出接入下一级，收集 trace。 |
...
3. `Pipeline` 用完整候选池作为种子输入，按序执行每个算子，逐级收集 `TraceEntry`。
...
- **一切皆算子。** 统一的 `run(batch) → batch` 接口加一个统一的执行处。正是这一点让流水线既可
